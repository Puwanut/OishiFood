let requestURL = 'json/media_center.json';
...
function dataReportStatus(data) {
    ...
    for(i; i < data.length; i++){
        ...
        meterial.insertAdjacentHTML("beforeend",
        "<p class='font-regular' id='date'><i class='fas fa-calendar-alt'></i>" + data[i].date +"</p>");
        meterial.insertAdjacentHTML("beforeend",
        "<div class='font-light' id='meterial_infront'>" + data[i].meterial_infront + "</div>");
        meterial.insertAdjacentHTML("beforeend",
        "<a href='#full_meterial" + i +"' data-toggle='modal' data-target='#full_meterial" + i + "' class='btn-seemore'><i class='fa fa-angle-right' aria-hidden='true' style='font-weight: 501;'></i><span style='font-size: 12px;'>ดูเพิ่มเติม</span></a>");
